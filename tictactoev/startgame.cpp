...
Startgame::Startgame(QWidget *parent, std::string p1, string p2, int mode,
    ...
    : QDialog(parent),
      ...
      game_mode_(mode) {
  ui->setupUi(this);
  if (musicPlayer1 == nullptr) musicPlayer1 = new QMediaPlayer(this);
  if (audioOutput1 == nullptr) audioOutput1 = new QAudioOutput(this);
...
void Startgame::AIFirst() {
  ...
  current_player_ =
      (current_player_ == ai_choice_) ? human_choice_ : ai_choice_;
}
...
void Startgame::StartNextRound() {
  ...
  if (game_mode_ == 2 && ai_choice_ == Player::X) Startgame::AIFirst();
}
